...
როგორც ხედავთ, უბრალოდ რომ ბოლო 39 კვირა აგვეღო, მხოლოდ Super Bowl-ს და labor day-ს შეიცავს, ორ ისეთ ჰოლიდეის, რომელსაც თთქმის ეფექტი არ აქვს. აქ არ შედის Thanksgiving და Christmas, რომლებიც ყველაზე მნიშვნელოვანია score-ისთვის.
...
გავაკეთე სტანდარტულად, როგორც ეს დასაწყისში, EDA ანალიზში ვთქვით.
...
**გადაწყვეტილება:** შეუვსებელი lag მნიშვნელობები, საერთოდ არ შევავსე, რადგან LightGBM-ს შეუძლია ამასთან გამკლავება თვითონ (ტრენინგის დროს სწავლობს).
...
**პირველი ცდა (არასწორი მეთოდი):** გამოვიყენე "split-based" importance, რაც ითვლის feature რამდენჯერ იყო გამოყენებული ხის სპლიტისთვის. ეს მეთოდი არასწორია, რადგან feature-ები, რომლებსაც  ბევრი შესაძლო მნიშვნელობა აქვთ (მაგალითად, ტემპერატურა) გამოიყენება მეტ სპლიტში შემთხვევით, თუნდაც ეს სპლიტი დიდად არ ეხმარებოდეს. ამ მეთოდით წავშალე 6 feature და უარესი შედეგები მივიღე.

![](images/lightgbm_split_based.png)

**მეორე ცდა (სწორი):** გამოვიყენე "gain-based" improtance, რაც ზომავს, feature-მა რამდენად შეამცირა prediction error. ეს მაძლევს უფრო გონივრულ რანკინგს.
...
გამოვიყენე 3-fold time-based cross-validation (თითოეული ფოლდი იყენებს ძველ მონაცემებს მომავლის პროგნოზისთვის და არასდროს ხდება პრიქით).
...
ამის შემდეგ, გადავწყვიტე უფრო მეტად გამეტესტა საუკეთესო კონფიგურაციის გარშემო (num_leaves, reg_alpha და reg_lambda-ის 27 კომბინაცია). აქ არ გამოვიყენე GridSearchCV, რადგან ამას არ შეუძლია დანახვა რომელი კვირა არის ჰოლიდეი და ოპტიმიზაციას გააკეთებდა უბრალოდ MAE-ს მიხედვით. მე გავაკეთე ჩემი search loop, გამოვიყენე, ცხადია, WMAE.
...
ჩანს, რომ ერორი მკვეთრად იზრდება 4-8 კვირაში (Thanksgiving და Christmas), რაც ამტკიცებს, რომ compounding პრობლემა ყველაზე ცუდია ჰოლიდეიების დროს.
...
ამ სექციაში ვისაუბრებ XGBoost-ზე. რადგანაც XGBoost-ც არის ხის მოდელი, როგორც LightGBM, ბევრი გადაწყვეტილება (cleaning, features, ზოგადი სტრატეგია) იგივეა. ყველა ექსპერიმენტი დალოგილია XGboost_Training-ში.
...
LightGBM-დან უკვე ვისწავლეთ, რომ split-based importance არსწორი იყო. XGBOOST-ისთვის, ეს შეცდომა გავითვალისწინე და პირდაპირ gain-based impoertance გამოვიყენე.
...
**საინტერესო განსხვავება LightGBM-სგან:**  

LightGBM-ში, ყველა feature-ის დატოვება იყო საუკეთესო გადაწყვეტილება. XGBoost-ში 8 lowest-gain feature-ების (has_markdown, month, Temperature, CPI, Unemployment, Fuel_Price, rolling_std_12) ამოღება დაგვეხმარა. ეს გვიჩვენებს, რომ იგივე გადაწყვეტილებას შეიძლება საწინააღმდეგო შედეგი ჰქონდეს სხვადახსხვა მოდელზე.
...
**განსხვავება LightGBM-სგან:** XGboost-ში shallow და deliberate_underfit ორივე კონსისტენტურად ცუდი იყო ყველაგან, ანდერფიტში იყვნენ. ამან მაჩვენა, რომ xgboost-ს ამ დატაზე სჭირდებოდა კარგი სიღრმეები, სიმარტივე აქ უფრო აფუჭებდა საქმეს, ვიდრე კომპლექსურობა.
...
### Skipping Recursive Forecasting

LightGBM-ში აღმოვაჩინეთ რეკურსიული forecasting-ის პრობლემა, რომელსაც აქ აღარ გავიმეორებ, ცხადია. compounding error ამ შემთხვევაში არაა დამოკიდებული თუ რომელ ხის ალგორითმს გამოვიყენებ. XGboost ზუსტად იგივე sales_lag_1/sales_lag_2-ს იყენებს, იგივე პრობლემით.
...
ARIMA არის AutoRegressive Integrated Moving Average. მას აქვს სამი კონფიგურაცია: (p, d, q).
...
**LightGBM Kaggle: Public 2642.17, Private 2751.90**
...
<img width="1968" height="456" alt="image" src="https://github.com/user-attachments/assets/6934b94c-c561-4535-97a2-76daf80d743c" />
